...
function shouldRedirectBackendUi() {
    // Diubah menjadi FALSE agar Railway melayani UI lokalnya sendiri.
    return String(process.env.BACKEND_UI_REDIRECT || 'false').toLowerCase() === 'true';
}
...
async function sendViaWhatsAppWeb({ to, message, image }) {
    ...
    if (image) {
        // Mendukung data URL (data:image/png;base64,...)
...
app.post('/api/whatsapp/send', async (req, res) => {
    ...
    if (!to || (!message && !image)) {
        return res.status(400).json({
            success: false,
            error: 'Body `to` dan salah satu dari `message`/`image` wajib diisi',
        });
    }
    ...
    if (transport === 'web') {
        try {
            const result = await sendViaWhatsAppWeb({ to, message, image });
            return res.status(200).json({
                success: true,
                mode: 'web',
...
// Melayani UI secara lokal dari folder public.
