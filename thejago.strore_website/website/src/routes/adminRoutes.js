...
const { getAllDocs, countDocs, createDoc, updateDoc, deleteDoc } = require('../services/knowledgeService');
const { embedText } = require('../services/embeddings');
const { syncGoogleReviews } = require('../services/googlePlaces');
...
router.post('/google-reviews/sync', requireAdmin, async (req, res, next) => {
  try {
    const body = req.body || {};
    ...
    res.json({ ok: true, sync: result, stored });
  } catch (err) {
    next(err);
  }
});
...
// Knowledge base docs for IVA
router.get('/kb/docs', requireAdmin, async (req, res, next) => {
  try {
    const limit = Math.min(Number(req.query.limit || 50), 200);
    const skip = Math.max(0, Number(req.query.skip || 0));
    const docs = await getAllDocs({ limit, skip });
...
router.put('/kb/docs/:id', requireAdmin, async (req, res, next) => {
  try {
    ...
    if (content !== undefined) {
      ...
      try {
        const emb = await embedText(update.content);
        if (emb) update.embedding = emb.vector;
      } catch (e) {
        console.warn('Embedding update failed:', e);
      }
    }
    if (isActive !== undefined) update.isActive = Boolean(isActive);

    const doc = await updateDoc(req.params.id, update);
...
router.delete('/kb/docs/:id', requireAdmin, async (req, res, next) => {
  try {
...
router.post('/kb/reindex', requireAdmin, async (req, res, next) => {
  try {
    const docs = await getAllDocs({ limit: 1000 });
    let updated = 0;
    for (const d of docs) {
      ...
      await updateDoc(d.id, { embedding: emb.vector });
      updated++;
    }
