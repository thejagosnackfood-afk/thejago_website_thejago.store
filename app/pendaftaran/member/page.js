...
import { User, Smartphone, MapPin, Send, CheckCircle2, AlertCircle, Database } from 'lucide-react'
...
export default function PendaftaranMemberPage() {
  ...
  useEffect(() => {
    // Generate ID Member otomatis jika kosong (TahunBulanHari + 4 digit random)
    const date = new Date()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 1. SINKRONISASI KE DATABASE (SUPABASE)
      // Kita asumsikan ada tabel 'members' atau kita gunakan tabel yang sesuai kebutuhan Anda
      ...
      if (dbError) {
        console.warn('Supabase Insert Error (mungkin tabel belum ada):', dbError.message)
        // Tetap lanjut kirim WA meskipun DB gagal jika Anda mau, 
        // atau stop di sini. Kita pilih lanjut dengan peringatan.
      }
      ...
      if (!res.ok) throw new Error('Gagal mengirim WhatsApp (Server Railway Offline)')
    ...
    <div className="__className_862193 min-h-screen bg-[#FDFCFB] text-[#1A1A1A] font-sans pb-20">
      ...
      <main className="max-w-6xl mx-auto px-4 mt-10 text-left">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          {/* LEFT: MANUAL INPUT (DENGAN SYNC DATABASE) */}
          <div className="lg:col-span-7 space-y-8">
            <div className="bg-white p-10 rounded-[40px] border border-gray-100 shadow-sm space-y-8">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-black text-gray-900 uppercase italic tracking-tighter">Pendaftaran Manual</h2>
                  <p className="text-xs text-gray-400 font-bold italic mt-1">Data akan otomatis disinkronkan ke Database & WhatsApp.</p>
                </div>
                <div className="bg-green-50 px-4 py-2 rounded-2xl flex items-center gap-2 border border-green-100">
                  <Database className="w-3 h-3 text-green-600" />
                  <span className="text-[9px] font-black text-green-600 uppercase tracking-widest text-left">Sync Active</span>
                </div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-2">
                  ...
                  <input name="fullName" required value={formData.fullName} onChange={handleChange} className="w-full h-14 bg-gray-50 border border-gray-100 rounded-2xl px-6 text-sm font-black outline-none focus:bg-white focus:border-red-200 transition-all" placeholder="Contoh: Budi Santoso" />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-4">Nomor WhatsApp</p>
                ...
                <button type="submit" disabled={loading} className="w-full h-16 bg-red-600 text-white rounded-[2rem] text-xs font-black uppercase tracking-[0.2em] shadow-xl shadow-red-100 hover:bg-gray-900 transition-all flex items-center justify-center gap-3">
                  {loading ? 'Menyimpan...' : <><Send className="w-4 h-4" /> Simpan & Kirim WhatsApp</>}
                </button>
              </form>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 space-y-8 sticky top-32">
            ...
            <div className="w-full aspect-[1.6/1] bg-red-800 rounded-[20px] shadow-2xl relative overflow-hidden text-white group hover:scale-[1.02] transition-transform duration-500">
               ...
               <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -mr-20 -mt-20 blur-3xl text-left"></div>
               <div className="relative z-10 p-6 h-full flex flex-col justify-between">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="text-right flex flex-col items-end">
                      <p className="text-[8px] font-black uppercase tracking-[0.3em] opacity-60 mb-1 text-right">Kartu Member</p>
                      ...
                      <p className="text-sm font-black tracking-tighter mt-1 text-right">{formData.memberId || '2026xxxx'}</p>
                    </div>
                  </div>
            ...
            {/* CARD BACK */}
            <div className="w-full aspect-[1.6/1] bg-white rounded-[20px] shadow-lg border border-gray-100 relative p-6 flex flex-col gap-4">
               ...
               <div className="flex gap-3">
                  <div className="w-1 h-auto bg-red-600 rounded-full opacity-30 text-left"></div>
                  <div>
